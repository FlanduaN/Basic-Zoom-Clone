...
#Graphical User Interfacec

window = tk.Tk()
window.title("LocalCalls v0.0.1 Alpha")
window.geometry('300x200')

label_target_ip = tk.Label(window, text="Targel IP")
label_target_ip.pack()

text_target_ip = tk.Text(window, height=1)
text_target_ip.pack()

btn_listen = tk.Button(window, text="Start Listeninig", width=50, command=start_listening)
btn_listen.pack(anchor=tk.CENTER, expand=True)

btn_camera = tk.Button(window, text="Start Camera", width=50, command=start_camera_stream)
btn_camera.pack(anchor=tk.CENTER, expand=True)

btn_stream = tk.Button(window, text="Start Screen Sharing", width=50, command=start_screen_stream)
